...
export default function ProfileScreen() {
  ...
  const loadP=useCallback(async()=>{try{const[m,f]=await Promise.all([api.getMe(),api.getFavorites()]);setHasBnet(m.has_bnet);setBtag(m.battletag);setFavs(f.characters);}catch{}finally{setLoading(false);}},[]);
    ...
    <SafeAreaView style={z.safe} edges={['top']}>
      <ScrollView contentContainerStyle={z.content}>
        ...
        <Card variant={hasBnet?'gold':'default'}>
          <View style={z.bRow}><Ionicons name={hasBnet?'shield-checkmark':'shield-outline'} size={24} color={hasBnet?colors.gold.primary:colors.text.tertiary}/><View style={z.bInfo}><Text style={z.bTag}>{hasBnet?btag:'Link Battle.net'}</Text><Text style={z.bSub}>{hasBnet?'Account linked':'Optional — enables private profiles'}</Text></View>{!hasBnet&&<Pressable onPress={bnetLink} style={z.linkBtn}><Text style={z.linkBtnT}>Link</Text></Pressable>}</View>
        </Card>
        <View style={z.sec}><Text style={z.secT}>Search Character</Text>
          <View style={z.sRow}><TextInput style={[z.sIn,{flex:2}]} value={realm} onChangeText={setRealm} placeholder="Realm slug" placeholderTextColor={colors.text.tertiary} autoCapitalize="none" autoCorrect={false}/><TextInput style={[z.sIn,{flex:3}]} value={cName} onChangeText={setCName} placeholder="Character name" placeholderTextColor={colors.text.tertiary} autoCapitalize="none" autoCorrect={false} onSubmitEditing={search}/><Pressable onPress={search} style={[z.sBtn,(!realm.trim()||!cName.trim())&&z.sBtnD]} disabled={!realm.trim()||!cName.trim()||searching}>{searching?<ActivityIndicator size="small" color={colors.bg.primary}/>:<Ionicons name="search" size={16} color={colors.bg.primary}/>}</Pressable></View>
        </View>
        {result&&<Card variant="arcane"><View style={z.rRow}>{result.avatar_url?<Image source={{uri:result.avatar_url}} style={z.av}/>:<View style={[z.av,z.avPh]}><Ionicons name="person" size={20} color={colors.text.tertiary}/></View>}<View style={z.rInfo}><Text style={z.rName}>{result.name}</Text><Text style={z.rMeta}>Lv {result.level} <Text style={{color:colors.classColor[result.class]||colors.text.primary}}>{result.race} {result.class}</Text></Text><Text style={z.rRealm}>{result.realm}</Text>{result.mount_count!=null&&<Text style={z.rMounts}>{result.mount_count} mounts</Text>}</View><Pressable onPress={addFav} style={z.favBtn}><Ionicons name="heart-outline" size={20} color={colors.fire.primary}/></Pressable></View></Card>}
        <View style={z.sec}><Text style={z.secT}>Favorites <Text style={z.secC}>({favs.length})</Text></Text>
          {favs.length===0&&!loading?<View style={z.eFav}><Ionicons name="heart-outline" size={28} color={colors.text.tertiary}/><Text style={z.eFavT}>No favorites</Text></View>:
          <View style={z.fList}>{favs.map(c=><Card key={c.id} variant={c.is_primary?'gold':'default'}><View style={z.fRow}>{c.avatar_url?<Image source={{uri:c.avatar_url}} style={z.fAv}/>:<View style={[z.fAv,z.avPh]}><Ionicons name="person" size={16} color={colors.text.tertiary}/></View>}<View style={z.fInfo}><Text style={z.fName}>{c.character_name}</Text><Text style={[z.fClass,{color:colors.classColor[c.class_name]||colors.text.primary}]}>{c.race_name} {c.class_name}</Text><Text style={z.fRealm}>{c.realm_slug}</Text></View><View style={z.fActs}>{c.is_primary&&<View style={z.mainB}><Text style={z.mainT}>MAIN</Text></View>}<Pressable onPress={()=>rmFav(c.id,c.character_name)} hitSlop={8}><Ionicons name="close-circle" size={18} color={colors.text.tertiary}/></Pressable></View></View></Card>)}</View>}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
