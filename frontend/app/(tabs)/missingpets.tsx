...
import { PET_REQUIREMENTS, checkCollectionReqs, describeReq, ReqCheck } from '../../data/collectionRequirements';

type PetWithReqs = GameItem & { reqCheck?: ReqCheck };

export default function MissingPetsScreen() {
  const router = useRouter();
  const {
    collectedPetIds, selectedChar,
    characterClass, characterRace, reputationStandings, completedAchievementIds, characterProfessions,
  } = useApp();
  const [allPets, setAllPets] = useState<GameItem[]>([]);
  ...
  const load = useCallback(async () => {
    try { setAllPets((await api.getAllPets()).pets); } catch {} finally { setLoading(false); }
  }, []);
  ...
  const { missing, unobtainableCount, totalInGame } = useMemo(() => {
    let unobtCount = 0;
    const totalInGame = allPets.length;
    const items: PetWithReqs[] = allPets
      .filter(p => !collectedPetIds.has(p.id))
      .map(p => {
        const reqs = PET_REQUIREMENTS[p.id];
        const reqCheck = selectedChar && reqs ? checkCollectionReqs(reqs, charData) : undefined;
        return { ...p, reqCheck };
      })
      .filter(p => {
        if (p.reqCheck) {
          if (p.reqCheck.unmet.some(r => r.type === 'unobtainable')) { unobtCount++; if (hideUnobtainable) return false; }
          if (p.reqCheck.unmet.some(r => r.type === 'class')) return false;
          if (hideUnmetReqs && !p.reqCheck.met) return false;
        }
        return true;
      });
    return { missing: items, unobtainableCount: unobtCount, totalInGame };
  }, [allPets, collectedPetIds, selectedChar, charData, hideUnobtainable, hideUnmetReqs]);
      ...
      <FlatList data={missing} keyExtractor={i => String(i.id)} contentContainerStyle={z.list}
        ...
        ListHeaderComponent={
          <View style={z.hdr}>
            ...
            <Text style={z.title}>Missing Pets</Text>
            <Text style={z.sub}>{selectedChar ? selectedChar.display.split('-')[0] : 'All pets'}</Text>
            <Card variant="gold" style={z.sc}>
              <View style={z.sr}>
                <View style={z.st}><Text style={z.sn}>{collectedPetIds.size}</Text><Text style={z.sl}>COLLECTED</Text></View>
                <View style={z.sd} />
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[z.row, isUnobt && z.rowDim]}>
              <View style={z.icon}><Ionicons name="paw" size={14} color={colors.fel.primary} /></View>
              <View style={z.info}>
        ...
        ListEmptyComponent={!loading ? (
          <View style={z.empty}><Ionicons name="paw" size={40} color={colors.gold.dim} /><Text style={z.emptyT}>All pets collected!</Text></View>
...
const z = StyleSheet.create({
  ...
  title: { ...typography.display, color: colors.fel.primary },
  ...
  rowDim: { opacity: 0.5 },
  icon: { width: 28, height: 28, borderRadius: radii.sm, backgroundColor: colors.fel.primary + '18', alignItems: 'center', justifyContent: 'center' },
  info: { flex: 1, gap: 1 },
