...
import { TITLE_REQUIREMENTS, checkCollectionReqs, describeReq, ReqCheck } from '../../data/collectionRequirements';

type TitleWithReqs = GameItem & { reqCheck?: ReqCheck };

export default function MissingTitlesScreen() {
  const router = useRouter();
  const {
    collectedTitleIds, selectedChar,
    characterClass, characterRace, reputationStandings, completedAchievementIds, characterProfessions,
  } = useApp();
  const [allTitles, setAllTitles] = useState<GameItem[]>([]);
  ...
  const load = useCallback(async () => {
    try { setAllTitles((await api.getAllTitles()).titles); } catch {} finally { setLoading(false); }
  }, []);
  ...
  const { missing, unobtainableCount, totalInGame } = useMemo(() => {
    let unobtCount = 0;
    const totalInGame = allTitles.length;
    const items: TitleWithReqs[] = allTitles
      .filter(t => !collectedTitleIds.has(t.id))
      .map(t => {
        const reqs = TITLE_REQUIREMENTS[t.id];
  ...
  }, [allTitles, collectedTitleIds, selectedChar, charData, hideUnobtainable, hideUnmetReqs]);
      ...
      <FlatList data={missing} keyExtractor={i => String(i.id)} contentContainerStyle={z.list}
        ...
        ListHeaderComponent={
          <View style={z.hdr}>
            ...
            <Text style={z.title}>Missing Titles</Text>
            <Text style={z.sub}>{selectedChar ? selectedChar.display.split('-')[0] : 'All titles'}</Text>
            <Card variant="gold" style={z.sc}>
              <View style={z.sr}>
                <View style={z.st}><Text style={z.sn}>{collectedTitleIds.size}</Text><Text style={z.sl}>COLLECTED</Text></View>
                <View style={z.sd} />
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[z.row, isUnobt && z.rowDim]}>
              <View style={z.icon}><Ionicons name="bookmark" size={14} color={colors.arcane.primary} /></View>
              <View style={z.info}>
        ...
        ListEmptyComponent={!loading ? (
          <View style={z.empty}><Ionicons name="bookmark" size={40} color={colors.gold.dim} /><Text style={z.emptyT}>All titles collected!</Text></View>
...
const z = StyleSheet.create({
  ...
  title: { ...typography.display, color: colors.arcane.primary },
  ...
  rowDim: { opacity: 0.5 },
  icon: { width: 28, height: 28, borderRadius: radii.sm, backgroundColor: colors.arcane.primary + '18', alignItems: 'center', justifyContent: 'center' },
  info: { flex: 1, gap: 1 },
