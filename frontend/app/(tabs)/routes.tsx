/**
 * Planner — Auto-generates farm routes from missing mounts.
 * Groups by zone, step-through run mode, auto-plan feature.
 */
...
// Known zones for auto-planning mount farms
...
export default function RoutesScreen() {
  ...
  const routes=useMemo(()=>{
    ...
    return Array.from(map.entries()).map(([zone,zt])=>({zone,tasks:zt.sort((a,b)=>a.sort_order-b.sort_order),completed:zt.filter(t=>t.completed).length})).sort((a,b)=>(a.completed/a.tasks.length)-(b.completed/b.tasks.length));
  },[tasks]);
  ...
  // Auto-plan: find missing farmable mounts not already in tasks
  const autoPlan=async()=>{
    ...
    const missing=mounts.filter(m=>m.source_type&&farmable.includes(m.source_type)&&!existingMountIds.has(m.id));

    if(missing.length===0){Alert.alert('All Planned','All farmable missing mounts are already in your task list!');return;}

    // Take top 20 easiest first (vendor, quest, achievement, rep, then the rest)
  ...
  return (
    <SafeAreaView style={z.safe} edges={['top']}>
      <ScrollView contentContainerStyle={z.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.gold.primary} progressBackgroundColor={colors.bg.secondary}/>}>
        <Text style={z.title}>Farm Planner</Text>
        <Text style={z.sub}>Auto-plan routes or manage your farm runs</Text>
      ...
      <Modal visible={activeRoute!==null} animationType="slide" transparent onRequestClose={()=>setActiveRoute(null)}>
        <View style={z.runBd}>
          <View style={z.runSheet}>
            <View style={z.runH}/>
            {activeRoute&&<>
              <View style={z.runHdr}><View><Text style={z.runZone}>{activeRoute.zone}</Text><Text style={z.runProg}>Step {runStep+1} of {activeRoute.tasks.length}</Text></View><Pressable onPress={()=>setActiveRoute(null)} style={z.runClose}><Ionicons name="close" size={20} color={colors.text.secondary}/></Pressable></View>
              ...
              <View style={z.runTask}>
                <View style={[z.runCircle,activeRoute.tasks[runStep]?.completed&&z.runCircleD]}>{activeRoute.tasks[runStep]?.completed?<Ionicons name="checkmark" size={28} color={colors.bg.primary}/>:<Text style={z.runNum}>{runStep+1}</Text>}</View>
                <Text style={z.runTitle}>{activeRoute.tasks[runStep]?.title}</Text>
                {activeRoute.tasks[runStep]?.source_type&&<View style={z.runSrc}><Text style={z.runSrcT}>{activeRoute.tasks[runStep].source_type}</Text></View>}
              </View>
              <View style={z.runActs}>
                <Pressable onPress={()=>setRunStep(Math.max(0,runStep-1))} style={[z.runNav,runStep===0&&z.runNavD]} disabled={runStep===0}><Ionicons name="arrow-back" size={18} color={runStep===0?colors.text.tertiary:colors.text.primary}/></Pressable>
                <Pressable onPress={()=>{toggle(activeRoute.tasks[runStep].id);if(runStep<activeRoute.tasks.length-1)setRunStep(runStep+1);}} style={[z.runComp,activeRoute.tasks[runStep]?.completed&&z.runCompD]}>
                  <Ionicons name={activeRoute.tasks[runStep]?.completed?'checkmark-done':'checkmark'} size={22} color={colors.bg.primary}/><Text style={z.runCompT}>{activeRoute.tasks[runStep]?.completed?'Done!':'Complete'}</Text>
                </Pressable>
                <Pressable onPress={()=>setRunStep(Math.min(activeRoute.tasks.length-1,runStep+1))} style={[z.runNav,runStep>=activeRoute.tasks.length-1&&z.runNavD]} disabled={runStep>=activeRoute.tasks.length-1}><Ionicons name="arrow-forward" size={18} color={runStep>=activeRoute.tasks.length-1?colors.text.tertiary:colors.text.primary}/></Pressable>
              </View>
            </>}
          </View>
        </View>
